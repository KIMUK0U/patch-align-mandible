...
class DentalTextCache:
    ...
    def encode_raw_texts(self, texts: list[str]) -> torch.Tensor:
        """テキストのリストを (K, D) Tensor にエンコードする。

        これが Stage 3 学習ループの主要インタフェース。
...
def build_clip_and_cache(
    ...
    Parameters
    ----------
    clip_model_name : 例 "ViT-bigG-14"
    clip_pretrained : 例 "laion2b_s39b_b160k"
    device          : torch.device
    capacity        : キャッシュ容量
    ...
    print(
        f"[TextCache] CLIP {clip_model_name}/{clip_pretrained} をロード"
        f"  テキスト次元: {clip_model.text_projection.shape[-1] if hasattr(clip_model, 'text_projection') else 'N/A'}"
    )
