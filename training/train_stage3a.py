...
def train_one_epoch(
    ...
    for batch in tqdm(loader, desc=f"Epoch {epoch} [Train]", leave=False):
        ...
        with torch.no_grad():
            ...
            patch_feat_t = proj(patch_emb_t)                          # (B, G, D) — Teacherの proj

        # CLIP テキスト特徴を取得 (バッチ内全サンプルのラベル名を統合)
        all_names: list[str] = []
        name_offset: list[int] = [0]
        for s in batch:
            all_names.extend(s["label_names"])
            name_offset.append(len(all_names))

        if not all_names:
            continue

        # ユニークなテキストだけエンコードして再利用
...
def main():
    ...
    train_ds = DentalPatchDataset(
        ...
        text_augment  = False,       # Stage 3a はテキスト固定
    ...
    logger.info("次: train_stage3b.py --stage3a_ckpt <このパス> を実行してください。")
