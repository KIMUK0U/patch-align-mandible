...
def _load_ckpt(model: nn.Module, proj: nn.Module, path: str, device: torch.device) -> None:
    ...
    if proj_state is not None and proj is not None:
        ...
        # 変更後
        current_proj_state = proj.state_dict()
        filtered = {
            k: v for k, v in proj_state.items()
            if k in current_proj_state and v.shape == current_proj_state[k].shape
        }
        proj.load_state_dict(filtered, strict=False)
        skipped = set(proj_state.keys()) - set(filtered.keys())
        if skipped:
            print(f'[Stage3] proj: 形状不一致のためスキップ → {skipped}')
            # proj.weight → proj.proj.weight (または形状表示を省略)
            inner = getattr(proj, 'proj', None) or getattr(proj, 'linear', None)
            if inner is not None:
                print(f'[Stage3] proj をランダム初期化 (out={inner.weight.shape[0]})')
            else:
                print(f'[Stage3] proj をランダム初期化')
    print(f"[Stage3] チェックポイントをロード: {path}")


def get_clip_text_module(clip_model: nn.Module) -> nn.Module:
    """BiomedCLIP (HFTextEncoder) か ViT-bigG-14 (CLIPTextTransformer) かを自動判定し
    テキスト Transformer モジュールを返す。train/eval 切り替えに使用する。
    """
    if hasattr(clip_model, "text"):
        return clip_model.text          # BiomedCLIP: HFTextEncoder
    return clip_model.transformer       # ViT-bigG-14: CLIPTextTransformer


def _freeze_hf_text_except_last(text_enc: nn.Module, n_last: int) -> None:
    """BiomedCLIP (HFTextEncoder / PubMedBERT) 用。最終 n_last BertLayer + proj を解凍。"""
    for p in text_enc.parameters():
        p.requires_grad = False
    bert_layers = text_enc.transformer.encoder.layer
    for i in range(max(0, len(bert_layers) - n_last), len(bert_layers)):
        for p in bert_layers[i].parameters():
            p.requires_grad = True
    if hasattr(text_enc, "proj"):
        for p in text_enc.proj.parameters():
            p.requires_grad = True


def _freeze_vit_text_except_last(clip_model: nn.Module, n_last: int) -> None:
    """ViT-bigG-14 (CLIPTextTransformer) 用。最終 n_last ResidualAttentionBlock + ln_final + text_projection を解凍。"""
    ...
    for i in range(max(0, len(resblocks) - n_last), len(resblocks)):
...
def freeze_text_encoder_except_last(clip_model: nn.Module, n_last: int = 1) -> None:
    """BiomedCLIP / ViT-bigG-14 の両方に対応したテキストエンコーダ部分凍結。

    BiomedCLIP (hasattr 'text'):
      clip_model.text.transformer.encoder.layer[-n_last:] + clip_model.text.proj を解凍
    ViT-bigG-14:
      clip_model.transformer.resblocks[-n_last:] + clip_model.ln_final + clip_model.text_projection を解凍
    """
    if hasattr(clip_model, "text"):
        _freeze_hf_text_except_last(clip_model.text, n_last)
        ref = clip_model.text
    else:
        _freeze_vit_text_except_last(clip_model, n_last)
        ref_params = list(clip_model.transformer.parameters())
        if hasattr(clip_model, "ln_final"):
            ref_params += list(clip_model.ln_final.parameters())
        if hasattr(clip_model, "text_projection") and clip_model.text_projection is not None:
            ref_params.append(clip_model.text_projection)
        n_train = sum(p.numel() for p in ref_params if p.requires_grad)
        n_total = sum(p.numel() for p in ref_params)
        print(
            f"[TextEncoder] 解凍: 最終 {n_last} 層 + ln_final + text_projection"
            f"  学習可能 {n_train:,} / 総 {n_total:,} パラメータ"
        )
        return

    n_train = sum(p.numel() for p in ref.parameters() if p.requires_grad)
    n_total = sum(p.numel() for p in ref.parameters())
    print(
        f"[TextEncoder] 解凍: 最終 {n_last} 層 + proj"
        f"  学習可能 {n_train:,} / 総 {n_total:,} パラメータ"
    )
