...
実行例 (v2 — Stage 3a 不要、Stage 2 チェックポイントから直接開始):
...
from phase3_models.stage3_model import build_stage3_models, freeze_encoder_except_last_block, freeze_text_encoder_except_last
...
def train_one_epoch(
    ...
    for batch in tqdm(loader, desc=f"Epoch {epoch} [Train]", leave=False):
        ...
        for b_idx, sample in enumerate(batch):
            ...
            # text_tune 時は gradient を流すため encode_with_grad を使用
            text_feats_b = text_cache.encode_with_grad(names_b)  # (K, D)
...
def main():
    parser = argparse.ArgumentParser("Stage 3b: 局所パッチ BCE + KD + EWC 学習 (v2 — Stage 3a 不要)")
    ...
    if text_tune_enabled:
        ...
        clip_model.text.train()   # visual encoder は触らない
    else:
        logger.info("[text_tune] 無効 — テキストエンコーダは凍結のまま")

    # ---- モデル (Stage 2 チェックポイントから直接初期化) ----
    ...
    if text_tune_enabled:
        ...
        text_params = [p for p in clip_model.parameters() if p.requires_grad]
    ...
    for epoch in range(start_epoch, n_epochs + 1):
        ...
        if epoch % eval_every == 0 or epoch == n_epochs:
            ...
            if text_tune_enabled:
                state["text_encoder"] = clip_model.text.state_dict()
        ...
        if epoch % save_every == 0:
            ...
            if text_tune_enabled:
                periodic["text_encoder"] = clip_model.text.state_dict()
            torch.save(periodic, ckpt_dir / f"epoch_{epoch:03d}.pt")
